...
DWORD WINAPI WorkThread(LPVOID arg)
{
	...
	if (GameManger::Instance->bullets[1]->View) {
		...
		// 데이터 전송 시작
		{
			...
			if (buffer_size != 0) {
				// 크기 보내주기
				send(client_sock[0], (char*)&buffer_size, sizeof(buffer_size), 0);
				send(client_sock[1], (char*)&buffer_size, sizeof(buffer_size), 0);

				// 패킷 보내주기
				send(client_sock[0], send_buffer, buffer_size, 0);
				send(client_sock[1], send_buffer, buffer_size, 0);
			}
		}
		// 데이터 전송 끝
